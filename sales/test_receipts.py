...
class ReceiptPrintTests(TestCase):
    ...
    def test_shared_receipt_escapes_names_and_preserves_edge_space(self):
        ...
        for prefix in ('', '/m'):
            ...
            self.assertContains(response, '@page { size:auto; margin:6mm 3mm 10mm; }')
            self.assertContains(response, 'width: calc(var(--receipt-width,71.9mm) - 6mm)')
            self.assertContains(response, '.receipt-content { padding: 0 !important; }')
